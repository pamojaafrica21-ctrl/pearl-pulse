...
<div class="pt-16 pb-16 bg-white">
    <div class="mx-auto max-w-3xl px-5 lg:px-8">
        @if($eyebrow)
            <p class="text-xs tracking-[0.22em] uppercase text-muted mb-3">{{ $eyebrow }}</p>
        @endif
        <h1 class="font-display text-5xl md:text-6xl text-charcoal">{{ $title }}</h1>
        @if($lead)
    ...
    <div class="mx-auto max-w-7xl px-5 lg:px-8">
        <div class="grid gap-10 md:grid-cols-3">
            @foreach($values as $value)
                <div>
                    <h2 class="font-display text-2xl md:text-3xl text-charcoal">{{ $value['title'] }}</h2>
                    @if($value['text'])
...
<section class="bg-white py-16 lg:py-24">
    <div class="mx-auto max-w-3xl px-5 lg:px-8">
        @if($approachHeading)
            <h2 class="font-display text-3xl md:text-4xl text-charcoal">{{ $approachHeading }}</h2>
        @endif
        @if($approachBody)
...
<section class="bg-white py-16">
    <div class="mx-auto max-w-7xl px-5 lg:px-8">
        <h2 class="font-display text-4xl text-charcoal mb-8">Our people</h2>
        <div class="grid gap-8 sm:grid-cols-3">
            @foreach($team as $member)
                <div>
                    ...
                    @endif
                    <h3 class="font-display text-2xl text-charcoal mt-4">{{ $member->name }}</h3>
                    <p class="text-sm text-muted">{{ $member->role }}</p>
                </div>
            @endforeach
        </div>
        <a href="{{ route('our-people') }}" class="inline-block mt-8 text-sm tracking-[0.14em] uppercase text-charcoal">Meet the team</a>
    </div>
</section>
@endif
