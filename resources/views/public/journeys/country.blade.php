...
<section class="bg-cream pt-16 pb-16">
    <div class="mx-auto max-w-7xl px-5 lg:px-8">
        <p class="text-xs tracking-[0.22em] uppercase text-gold mb-3">{{ $country->name }}</p>
        <h1 class="font-display text-5xl md:text-6xl text-forest">{{ $country->name }} journeys</h1>
        @if($country->teaser)
...
<section class="bg-cream pb-20">
    <div class="mx-auto max-w-7xl px-5 lg:px-8 grid gap-10 sm:grid-cols-2 lg:grid-cols-3">
        ...
        @empty
            <p class="text-muted">Journeys for {{ $country->name }} are being prepared. <a href="{{ route('plan') }}" class="text-forest underline">Plan a journey</a> and we will design one.</p>
        @endforelse
    </div>
</section>
