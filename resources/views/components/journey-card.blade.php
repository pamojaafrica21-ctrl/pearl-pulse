...
<a href="{{ route('journeys.show', $journey) }}" class="destination-card group">
    ...
    <div class="destination-card-meta">
        ...
        <h3 class="font-display text-2xl md:text-3xl text-charcoal mt-1 group-hover:text-charcoal transition-colors">
            {{ $journey->name }}
        </h3>
        @if($journey->teaser)
            <p class="mt-2 text-sm text-muted leading-relaxed line-clamp-3">{{ $journey->teaser }}</p>
        @endif
        <p class="mt-3 text-[11px] tracking-[0.14em] uppercase text-muted">Private proposal</p>
    </div>
</a>
